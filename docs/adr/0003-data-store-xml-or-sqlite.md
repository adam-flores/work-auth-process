...
- **The candidate success measures are queries** (BUSINESS_CASE.md §6): first-pass
...
- **A real deployment would not use either one.** Whichever we pick is a prototype stand-in
  ...
  involved use different reporting systems (BUSINESS_CASE.md §8). The prototype must not
